...
* Drop `nvm` installed with `brew` support as it's not supported by `nvm` itself (see https://github.com/creationix/nvm#important-notes). `nvm` standard installation is still supported though.
